...
public enum OptimumThreshold implements UniquePixelsBandOperator {
  ...
  public double[][] apply(final double[][] pixels) {
     ...
     */
    final double[][] internalPixels = Normalizer.INSTANCE.normalizeToDouble(pixels);

    // Step 1.
    final double[] relativeFrequencies = new double[ImageMatrix.getMaxNormalizedPixelValue() + 1];
    final double N = internalPixels.length * internalPixels[0].length;
    final double delta = 1d / N;
    //noinspection ForLoopReplaceableByForEach
    for (int y = 0; y < internalPixels.length; y++) {
      for (int x = 0; x < internalPixels[y].length; x++) {
        relativeFrequencies[(int) internalPixels[y][x]] += delta;
      }
    }
    ...
    cumulativeMedias[0] = 0 * relativeFrequencies[0]; // Zero (this is just to make explicit what we are doing).
    ...
    final int threshold = maxVariancesIndexes.get(thresholdIndex);
    return Threshold.apply(internalPixels, threshold);
  }
}
