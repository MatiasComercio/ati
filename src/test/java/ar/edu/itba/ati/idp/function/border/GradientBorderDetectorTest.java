...
public class GradientBorderDetectorTest {
  ...
  public Object[] testBehaviourParams() { // All border detectors with their parameters.
    return new Object[][] {
        {GradientBorderDetector.PREWITT, PrewittMask.newInstance()},
        {GradientBorderDetector.SOBEL, SobelMask.newInstance()}
    };
  }

  @Test
  @Parameters(method = "testBehaviourParams")
  public  <T extends RotatableMask<T>> void testBehaviour(final GradientBorderDetector detector,
                                                           final T mask) {
    // when
    final double[][] actualPixels =  detector.apply(pixels);

    // verify
    final UniquePixelsBandOperator op = GradientFilter.newInstance(mask);
